...
from app.ml_client import predict_sentiment_sync
...
# Utility: Normalize phone to E.164 format for Exotel
def normalize_e164(phone: str) -> str:
    """
    Normalize a phone number to E.164 format for Exotel.
    Handles Indian and international numbers robustly.
    """
    if not phone:
        return ""
    ...
    # If 10 digits, assume Indian mobile, add +91
    if len(phone) == 10:
        return "+91" + phone
    # If 12 digits and starts with '91', add +
    if len(phone) == 12 and phone.startswith("91"):
        return "+" + phone
    # If 11 digits and starts with '91', treat as error (not valid E.164)
    # If 11 digits and starts with '1', could be US, add +
...
def initiate_call(self, target_id: str, phone_number: str, call_id: str):
    ...
    with get_db_session_cm() as db:
        try:
            ...
            # Format phone number for Exotel (E.164)
            formatted_phone = normalize_e164(phone_number)
            formatted_exotel_phone = normalize_e164(EXOTEL_PHONE_NUMBER)
            ...
            call_data = {
                ...
                # 'Url': f'https://{APP_HOST}/exoml/greeting-record?token={EXOTEL_WEBHOOK_TOKEN}',
                'StatusCallback': f"https://{APP_HOST}/status_callback?token={EXOTEL_WEBHOOK_TOKEN}",
            ...
            logger.info(
                "Calling Exotel API: From=%s, Url=%s, CorrelationID=%s",
                call_data['From'],
                call_data['Url'],
                correlation_id,
            )
...
def _process_recording_emotion_job(call_id: str):
    ...
    try:
        with get_db_session_cm() as db:
            ...
            ml_result = predict_sentiment_sync(audio_bytes, filename=temp_path)
            emotion_label = ml_result.get('emotion', 'Neutral')
            sentiment = ml_result.get('sentiment', 'Neutral')
            
            # Map emotion to database emotion record
            ...
            logger.info(
                "Emotion processing completed for Call=%s -> %s (sentiment: %s)",
                call_id,
                emotion_label,
                sentiment
            )
